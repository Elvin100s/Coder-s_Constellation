...
export function renderProjectDetail(): HTMLElement {
  ...
  container.innerHTML = `
    <div class="max-w-4xl mx-auto px-2 sm:px-4 md:px-6 lg:px-8 py-4 sm:py-8">
      ...
      <!-- Project Header -->
      <div class="card p-4 sm:p-6 lg:p-8 mb-6">
        <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between mb-6 space-y-4 sm:space-y-0">
          <div class="flex-1 min-w-0">
            <h1 class="text-2xl sm:text-3xl lg:text-4xl font-bold bg-gradient-to-r from-neon-pink to-neon-blue bg-clip-text text-transparent mb-2 break-words">${project.title}</h1>
            <p class="text-gray-400 text-sm sm:text-base">Posted ${formatDate(project.createdAt)}</p>
          </div>
          ${project.githubRepo ? `
            <a href="${project.githubRepo}" target="_blank" rel="noopener noreferrer" 
               class="btn btn-secondary flex items-center space-x-2 w-full sm:w-auto text-sm sm:text-base px-3 sm:px-4 py-2">
